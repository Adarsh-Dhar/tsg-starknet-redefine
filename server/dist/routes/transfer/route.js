...
// Load vault ABI
const vaultAbiPath = path.join(process.cwd(), '../grass_vault/target/dev/grass_vault_GravityVault.contract_class.json');
let VAULT_ABI = [
    {
        name: 'transfer',
        type: 'function',
 ...
 */
router.post('/strk', async (req, res) => {
    try {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Transaction timeout - backend account may need funding')), 30000));
            const invokeResponse = await Promise.race([invokePromise, timeoutPromise]);
            ...
            else if (normalizedError.includes('transaction timeout') ||
