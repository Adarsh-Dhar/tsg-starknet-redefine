...
export async function verifyDelegationTransaction(
  ...
  try {
    ...
    if (Array.isArray(calldata)) {
      vaultInteraction = calldata.some((call: any) => 
        call.contract_address === VAULT_ADDRESS || 
 ...
 */
export async function getDelegationAmount(address: string): Promise<bigint | null> {
  try {
